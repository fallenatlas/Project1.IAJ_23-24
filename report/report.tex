...
\begin{document}
  ...
  performance when compared to other algorithms like djikstra. It's also a very modular algorithm, meaning its performance can be improved by using better data structures,
  ...
  The algorithm performs quite well, even in its most basic form. Most of the time is spent searching in 
  the closed set, because we start by searching it when processing a node's neighbors. Searching 
  ...
  f value. We chose to prioritize nodes with lower h cost, so that we pick the node that is closer to the goal. 
  This way, we can reduce the number of explored nodes by choosing a node over the other, instead of randomly exploring one of them.\\
  \subsection{Data}
  \begin{table}[h!]
      \parbox{.45\linewidth}{
        \centering
